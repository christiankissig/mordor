...
(** Get all values *)
let values s =
  Hashtbl.fold (fun _ v acc -> v :: acc) s []

(** Size *)
let size s = Hashtbl.length s

(** Clear set *)
let clear s = Hashtbl.clear s; s
...
let intersection s1 s2 =
  ...
  Hashtbl.iter (fun k v ->
    if Hashtbl.mem s2 k then Hashtbl.add result k v
  ) s1;
  result
...
let filter f s =
  let result = create () in
  Hashtbl.iter (fun _ v ->
    if f v then Hashtbl.add result v v
  ) s;
  result
...
let ifilter f s =
  ...
  Hashtbl.iter (fun k v ->
    if not (f v) then to_remove := k :: !to_remove
  ) s;
...
(** Iter *)
let iter f s =
  Hashtbl.iter (fun _ v -> f v) s

(** Fold *)
let fold f s init =
  Hashtbl.fold (fun _ v acc -> f acc v) s init

(** For all *)
let for_all f s =
  Hashtbl.fold (fun _ v acc -> acc && f v) s true

(** Exists *)
let exists f s =
  Hashtbl.fold (fun _ v acc -> acc || f v) s false
...
let cross s1 s2 =
  let result = create () in
  Hashtbl.iter (fun _ v1 ->
    Hashtbl.iter (fun _ v2 ->
      let pair = (v1, v2) in
...
(** Identity relation *)
let identity_relation s =
  map (fun x -> (x, x)) s

(** Inverse relation *)
let inverse_relation s =
  map (fun (a, b) -> (b, a)) s
...
let transitive_closure s =
  ...
  while !changed do
    ...
    List.iter (fun (a, b) ->
      List.iter (fun (c, d) ->
        if b = c && not (mem result (a, d)) then begin
...
(** Reflexive closure *)
let reflexive_closure domain s =
  union (identity_relation domain) s
...
let acyclic s =
  ...
  while !changed do
    ...
    List.iter (fun key ->
      if not (Hashtbl.mem right_map key) then begin
...
let is_irreflexive s =
  for_all (fun (a, b) -> a <> b) s

(** Equality *)
let equal s1 s2 =
  size s1 = size s2 &&
  for_all (fun v -> mem s2 v) s1
...
(** To string *)
let to_string s =
  let vals = values s |> List.map (fun v -> Printf.sprintf "%d" (Obj.magic v)) in

...
and Expr : sig
  ...
  let rec get_symbols = function
    ...
    | EBinOp (lhs, _, rhs) ->
        Value.get_symbols lhs @ Value.get_symbols rhs
    | EUnOp (_, rhs) -> Value.get_symbols rhs
    | EOr clauses ->
        List.flatten (List.map (fun c -> List.flatten (List.map get_symbols c)) clauses)
    | _ -> []
  ...
  let is_flat = function
    | EBinOp (lhs, op, rhs) ->
        ...
        not (match lhs with VExpression _ -> true | _ -> false) &&
        not (match rhs with VExpression _ -> true | _ -> false)
    | _ -> false
  
  let inverse = function
    | EBinOp (lhs, op, rhs) ->
        let inv_op = match op with
          | "=" -> "!="
          | "!=" -> "="
          | "<" -> ">="
          | ">=" -> "<"
  ...
  let rec subst e a b =
    ...
    match e with
    | EBinOp (lhs, op, rhs) ->
        let new_lhs = if eq lhs a then b else
          match lhs with
          | VExpression e -> VExpression (subst e a b)
          | _ -> lhs
        in
        let new_rhs = if eq rhs a then b else
          match rhs with
          | VExpression e -> VExpression (subst e a b)
          | _ -> rhs
        in
  ...
  let rec to_string = function
    ...
    | EBinOp (lhs, op, rhs) ->
        Printf.sprintf "(%s %s %s)" 
          (Value.to_string lhs) op (Value.to_string rhs)
    | EUnOp (op, rhs) ->
        Printf.sprintf "%s(%s)" op (Value.to_string rhs)
    | EOr clauses ->
        let clause_str = List.map (fun c ->
          String.concat " ∧ " (List.map to_string c)
        ) clauses in
        String.concat " ∨ " clause_str
  
  let rec flatten = function
    | EBinOp (lhs, "&&", rhs) ->
        let l_flat = match lhs with VExpression e -> flatten e | _ -> [EVar "?"] in
        let r_flat = match rhs with VExpression e -> flatten e | _ -> [EVar "?"] in
        l_flat @ r_flat
    | e -> [e]

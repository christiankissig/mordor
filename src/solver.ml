...
let get_var context name =
  try
    Hashtbl.find context.vars name
  with Not_found ->
...
let rec value_to_z3 context = function
  | VNumber n ->
      Z3.Arithmetic.Integer.mk_numeral_s context.ctx (Z.to_string n)
  | VSymbol s | VVar s ->
      get_var context s
  | VExpression e ->
      expr_to_z3 context e
  | VBoolean b ->
...
and expr_to_z3 context = function
  ...
  | EOr clauses ->
      let clause_exprs = List.map (fun clause ->
       ...
       | Some m ->
           ...
           Hashtbl.iter (fun name var ->
             ...
             | Some value ->
                 ...
                 with _ ->
                   Hashtbl.add bindings name (VVar str))
             | None -> ()
           ) solver.context.vars;
...
let concrete_value bindings var =
  try
    Some (Hashtbl.find bindings var)
  with Not_found -> None
...
let simplify_disjunction clauses =
  ...
  let clauses' = List.filter (fun clause ->

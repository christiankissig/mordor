...
(** Token types *)
type token_type =
  | NONATOMIC | RELAXED | RELEASE | ACQUIRE | STRONG | NORMAL | SC
  | FADD | CAS | IF | ELSE | WHILE | DO | SKIP | QDO | QWHILE
  | FENCE | MALLOC | FREE | LOCK | UNLOCK
  | LOAD | STORE | ALLOW | FORBID | NAME | VALUES
  | INT of Z.t
  | REGISTER of string
  | ATLOC of string
  | GLOBAL of string
  ...
  | BACKTICK of string
  | COMMENT of string
  | QUOTE | PLUS | MINUS | STAR | SLASH | ARROW | RARROW
  | AND | OR | PERCENT | NEQ | GEQ | GT | LEQ | LT
  | ASSIGN | EQ | LPAREN | RPAREN | LBRACE | RBRACE | LDBRACKET
  | COLONRLX | COLONREL | COLONACQ | COLONSC
  | COLONV | COLONVRLX | COLONVREL | COLONVACQ | COLONVSC
  | RDBRACKET | LBRACKET | RBRACKET | SEMICOLON | COMMA
  | BANG | DOT | HASH | AMPERSAND | CARET | PIPE | TILDE
  | IN | NOTIN | FORALL | PARALLEL
  | EOF
...
let current_char t =
  if t.index < String.length t.src then
    Some t.src.[t.index]
  else
    None
...
let make_token t typ =
  { typ; line = t.line; col = t.index - t.line_start + 1 }
...
let skip_whitespace t =
  while match current_char t with
    | Some c when is_whitespace c -> advance t; true
    | _ -> false
  do () done
...
let read_while t pred =
  ...
  while match current_char t with
    | Some c when pred c -> Buffer.add_char buf c; advance t; true
    | _ -> false
  do () done;
  Buffer.contents buf
...
let starts_with t str =
  ...
  t.index + len <= String.length t.src &&
  String.sub t.src t.index len = str

let try_keyword t =
  let keywords = [
    ("nonatomic", NONATOMIC); ("na", NONATOMIC);
    ("relaxed", RELAXED); ("rlx", RELAXED);
    ("release", RELEASE); ("rel", RELEASE);
    ("acquire", ACQUIRE); ("acq", ACQUIRE);
    ("strong", STRONG); ("normal", NORMAL);
    ("sequentially_consistent", SC); ("sc", SC);
    ("FADD", FADD); ("fadd", FADD);
    ("CAS", CAS); ("cas", CAS);
    ("if", IF); ("else", ELSE);
    ("while", WHILE); ("do", DO);
    ("skip", SKIP); ("qdo", QDO); ("qwhile", QWHILE);
    ("fence", FENCE); ("malloc", MALLOC); ("free", FREE);
    ("lock", LOCK); ("unlock", UNLOCK);
    ("load", LOAD); ("store", STORE);
    ("allow", ALLOW); ("forbid", FORBID);
    ("name", NAME); ("values", VALUES);
  ] in
  List.find_map (fun (word, tok) ->
    if starts_with t word then
      let next_idx = t.index + String.length word in
      if next_idx >= String.length t.src ||
         not (is_alnum t.src.[next_idx]) then
        Some (word, tok)
      else
        None
    else
...
let next_token t =
  ...
  if t.index >= String.length t.src then
    ...
    | None ->
        ...
        (* Registers *)
        | Some 'r' when (
            let next_idx = t.index + 1 in
            next_idx < String.length t.src &&
            is_alnum t.src.[next_idx]
          ) ->
            advance t;
        ...
        | Some '`' ->
            ...
            advance t; (* consume closing backtick *)
        ...
        | Some ':' when starts_with t ":rlx=" -> t.index <- t.index + 5; make_token t COLONRLX
        | Some ':' when starts_with t ":rel=" -> t.index <- t.index + 5; make_token t COLONREL
        | Some ':' when starts_with t ":acq=" -> t.index <- t.index + 5; make_token t COLONACQ
        | Some ':' when starts_with t ":sc=" -> t.index <- t.index + 4; make_token t COLONSC
        | Some ':' when starts_with t ":vrlx=" -> t.index <- t.index + 6; make_token t COLONVRLX
        | Some ':' when starts_with t ":vrel=" -> t.index <- t.index + 6; make_token t COLONVREL
        | Some ':' when starts_with t ":vacq=" -> t.index <- t.index + 6; make_token t COLONVACQ
        | Some ':' when starts_with t ":vsc=" -> t.index <- t.index + 5; make_token t COLONVSC
        | Some ':' when starts_with t ":v=" -> t.index <- t.index + 3; make_token t COLONV
        | Some ':' when starts_with t ":=" -> t.index <- t.index + 2; make_token t ASSIGN
        | Some '=' when starts_with t "=>" -> t.index <- t.index + 2; make_token t ARROW
        | Some '~' when starts_with t "~~>" -> t.index <- t.index + 3; make_token t RARROW
        | Some '|' when starts_with t "|||" -> t.index <- t.index + 3; make_token t PARALLEL
        | Some '&' when starts_with t "&&" -> t.index <- t.index + 2; make_token t AND
        | Some '|' when starts_with t "||" -> t.index <- t.index + 2; make_token t OR
        | Some '%' when starts_with t "%%" -> t.index <- t.index + 2; make_token t PERCENT
        | Some '!' when starts_with t "!=" -> t.index <- t.index + 2; make_token t NEQ
        | Some '>' when starts_with t ">=" -> t.index <- t.index + 2; make_token t GEQ
        | Some '<' when starts_with t "<=" -> t.index <- t.index + 2; make_token t LEQ
        | Some '[' when starts_with t "[[" -> t.index <- t.index + 2; make_token t LDBRACKET
        | Some ']' when starts_with t "]]" -> t.index <- t.index + 2; make_token t RDBRACKET
        
        (* Single character tokens *)
        | Some '\'' -> advance t; make_token t QUOTE
        | Some '+' -> advance t; make_token t PLUS
        | Some '-' -> advance t; make_token t MINUS
        | Some '*' -> advance t; make_token t STAR
        | Some '/' -> advance t; make_token t SLASH
        | Some '>' -> advance t; make_token t GT
        | Some '<' -> advance t; make_token t LT
        | Some '=' -> advance t; make_token t EQ
        | Some '(' -> advance t; make_token t LPAREN
        | Some ')' -> advance t; make_token t RPAREN
        | Some '{' -> advance t; make_token t LBRACE
        | Some '}' -> advance t; make_token t RBRACE
        | Some '[' -> advance t; make_token t LBRACKET
        | Some ']' -> advance t; make_token t RBRACKET
        | Some ';' -> advance t; make_token t SEMICOLON
        | Some ',' -> advance t; make_token t COMMA
        | Some '!' -> advance t; make_token t BANG
        | Some '.' -> advance t; make_token t DOT
        | Some '#' -> advance t; make_token t HASH
        | Some '&' -> advance t; make_token t AMPERSAND
        | Some '^' -> advance t; make_token t CARET
        | Some '|' -> advance t; make_token t PIPE
        | Some '~' -> advance t; make_token t TILDE
        
        | Some c ->
            failwith (Printf.sprintf "Tokenizer error at line %d, col %d: unexpected character '%c'"
              t.line (t.index - t.line_start + 1) c)
...
let current_token p =
  if p.index < List.length p.tokens then
    List.nth p.tokens p.index
  else
    { typ = EOF; line = 0; col = 0 }
...
let expect p expected =
  ...
  if tok.typ = expected then begin
    advance_parser p;
    tok
  end else
    failwith (Printf.sprintf "Parse error at line %d: expected %s"
      tok.line (match expected with EOF -> "EOF" | _ -> "token"))
...
type ast_stmt =
  ...
  | SCas of { register: string; params: ast_expr list; modes: mode list; assign: assign_info }
  | SFAdd of { register: string; params: ast_expr list; modes: mode list; assign: assign_info }
  | SRegisterStore of { register: string; expr: ast_expr }
  | SIf of { condition: ast_expr; body: ast_stmt list; else_body: ast_stmt list option }
  ...
  | SMalloc of { register: string; size: ast_expr; pc: int; label: string list }
...
type ast_assertion =
  | AOutcome of { outcome: string; condition: ast_expr; model: string option }
...
let rec parse_expr p ?(min_prec=0) () =
  let parse_atom () =
    ...
    | _ ->
        failwith (Printf.sprintf "Parse error: unexpected token in expression at line %d"
...
let parse_memory_order p =
  match (current_token p).typ with
  | NONATOMIC -> advance_parser p; Nonatomic
  | RELAXED -> advance_parser p; Relaxed
  | RELEASE -> advance_parser p; Release
  | ACQUIRE -> advance_parser p; Acquire
  | SC -> advance_parser p; SC
  | NORMAL -> advance_parser p; Normal
  | STRONG -> advance_parser p; Strong
  | _ -> Relaxed
...
let parse_assign p =
  ...
  | _ ->
      failwith "Expected assignment operator"

(** Parse parameter list *)
let parse_params p parse_elem =
  let _ = expect p LPAREN in
  let rec collect acc =
    if (current_token p).typ = RPAREN then begin
      advance_parser p;
      List.rev acc
    end else begin
      let elem = parse_elem () in
      let acc' = elem :: acc in
      if (current_token p).typ = COMMA then begin
        advance_parser p;
        collect acc'
      end else begin
        let _ = expect p RPAREN in
...
let rec parse_program p =
  let rec parse_stmts acc labels =
    ...
    | STAR ->
        ...
        let stmt = if labels <> [] then SLabeled { label = List.rev labels; stmt } else stmt in
        parse_stmts (stmt :: acc) []
    
    | GLOBAL g ->
        advance_parser p;
        (match (current_token p).typ with
         | COLONRLX | COLONREL | COLONACQ | COLONSC | COLONV | COLONVRLX | COLONVREL | COLONVACQ | COLONVSC | ASSIGN ->
             let assign = parse_assign p in
             let expr = parse_expr p () in
         ...
         | DOT ->
             ...
             (match (current_token p).typ with
              | STORE ->
                  ...
                  let mode = if List.length params > 1 then
    ...
    | REGISTER r ->
        ...
        (match (current_token p).typ with
         | GLOBAL g ->
             advance_parser p;
             let mode = match (current_token p).typ with
               | DOT ->
                   advance_parser p;
         ...
         | CAS ->
             advance_parser p;
             let all_params = parse_params p (fun () ->
               match parse_memory_order_opt p with
               | Some m -> `Mode m
               | None -> `Expr (parse_expr p ())
             ) in
             let modes = List.filter_map (function `Mode m -> Some m | _ -> None) all_params in
             let params = List.filter_map (function `Expr e -> Some e | _ -> None) all_params in
    ...
    | IF ->
        ...
        let _ = expect p RBRACE in
        let else_body = match expect_opt p ELSE with
          | Some _ ->
              let _ = expect p LBRACE in
    ...
    | LOCK ->
        advance_parser p;
        let global = match (current_token p).typ with
          | GLOBAL g -> advance_parser p; Some g
          | _ -> None
        in
    ...
    | _ ->
        failwith (Printf.sprintf "Unexpected token in program at line %d" tok.line)
  in
  ...
  let process_malloc stmts =
    List.fold_right (fun stmt acc ->
      ...
      | SGlobalStore { global; expr = EMalloc size; assign } ->
          p.malloc_index <- p.malloc_index - 1;
          let reg = Printf.sprintf "r%d" p.malloc_index in
          let malloc_stmt = SMalloc { register = reg; size; pc = 0; label = [] } in
          let store_stmt = SGlobalStore {
            global;
            expr = ERegister reg;
            assign
          } in
...
let parse_config p =
  ...
  (* Parse name *)
  if expect_opt p NAME <> None then begin
    let _ = expect p EQ in
    while not (List.mem (current_token p).typ [VALUES; LBRACKET; PERCENT; LDBRACKET]) do
      (match (current_token p).typ with
  ...
  (* Parse values *)
  if expect_opt p VALUES <> None then begin
    let _ = expect p EQ in
    let _ = expect p LBRACE in
    while (current_token p).typ <> RBRACE do
      (match (current_token p).typ with
       | INT n -> values := n :: !values; advance_parser p
       | _ -> advance_parser p);
  ...
  if !name <> "" || !values <> [] || !defacto <> [] || !constraint_ <> [] then begin
...
let rec parse_litmus p =
  ...
  let assertion = if expect_opt p PERCENT <> None then
    Some (parse_assertion p)
  else
    None
  in
...
and parse_assertion p =
  let parse_outcome () =
    match (current_token p).typ with
    | ALLOW -> advance_parser p; "allow"
    | FORBID -> advance_parser p; "forbid"
    | _ -> failwith "Expected allow or forbid"
  in
  
  let parse_check () =
    let _ = expect p LBRACKET in
    if expect_opt p RBRACKET <> None then
      (None, None)
    else
      let model = match (current_token p).typ with
        | GLOBAL g | REGISTER g -> Some g
        | _ -> None
      in
      (if model <> None then advance_parser p);
      let outcome = if expect_opt p EQ <> None then Some (parse_outcome ()) else None in
      let _ = expect p RBRACKET in
      (model, outcome)
  in
  
  if (current_token p).typ = LBRACKET then begin
    let (model, _) = parse_check () in
    AModel { model = Option.value model ~default:"" }
  end
  else if expect_opt p RARROW <> None then begin
...
let rec ast_expr_to_expr = function
  ...
  | EBinOp (l, op, r) ->
      EBinOp (VExpression (ast_expr_to_expr l), op, VExpression (ast_expr_to_expr r))
  | EUnOp (op, e) ->
      EUnOp (op, VExpression (ast_expr_to_expr e))
  | ETuple (e1, e2) ->
      (* Represent tuple as a special binop *)
      EBinOp (VExpression (ast_expr_to_expr e1), ",", VExpression (ast_expr_to_expr e2))

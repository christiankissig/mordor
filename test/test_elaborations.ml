...
let create_mock_context () =
  ...
  let pred _con _exprs ?ppo:_ () =
    Lwt.return (fun _e -> Uset.create ())
  in
...
let test_weaken_with_pwg () =
  ...
  check bool "weaken removes implied" true (List.length just'.p <= List.length just.p);
  Lwt.return_unit
...
let suite =
  ("Elaborations", [
    ...
    test_case "filter removes covered" `Quick (lwt_test test_filter_removes_covered);
    test_case "value_assign no variables" `Quick (lwt_test test_value_assign_no_variables);
    test_case "value_assign with variable" `Quick (lwt_test test_value_assign_with_variable);
    ...
    test_case "forward single justification" `Quick (lwt_test test_forward_single_justification);
    ...
    test_case "weaken preserves non-implied" `Quick (lwt_test test_weaken_preserves_non_implied);
    test_case "strengthen basic" `Quick (lwt_test test_strengthen_basic);
    test_case "strengthen disjoint predicates" `Quick (lwt_test test_strengthen_disjoint_predicates);
  ])

...
let test_value_constructors () =
  ...
  Alcotest.(check bool) "number is created" true (match n with VNumber _ -> true | _ -> false);
  Alcotest.(check bool) "symbol is created" true (match s with VSymbol _ -> true | _ -> false);
  Alcotest.(check bool) "var is created" true (match v with VVar _ -> true | _ -> false);
  Alcotest.(check bool) "boolean is created" true (match b with VBoolean _ -> true | _ -> false)
...
let test_value_subst () =
  ...
  Alcotest.(check bool) "no substitution on number" true
...
let test_expr_constructors () =
  ...
  Alcotest.(check bool) "num expr created" true (match num_expr with ENum _ -> true | _ -> false);
  Alcotest.(check bool) "var expr created" true (match var_expr with EVar _ -> true | _ -> false);
  Alcotest.(check bool) "binop expr created" true (match binop_expr with EBinOp _ -> true | _ -> false);
  Alcotest.(check bool) "unop expr created" true (match unop_expr with EUnOp _ -> true | _ -> false)
...
let test_expr_to_string () =
  ...
  Alcotest.(check bool) "binop string contains operator" true (String.contains s1 '+');
  Alcotest.(check bool) "unop string contains operator" true (String.contains s2 '-')
...
let test_expr_is_flat () =
  ...
  Alcotest.(check bool) "not flat with nested expr" false (Expr.is_flat not_flat)

let test_expr_inverse () =
  let eq = e_binop (v_var "x") "=" (v_num 5) in
  let neq = Expr.inverse eq in
  Alcotest.(check bool) "inverse of =" true
    (match neq with EBinOp (_, "!=", _) -> true | _ -> false);

  let lt = e_binop (v_var "x") "<" (v_num 5) in
  let gte = Expr.inverse lt in
  Alcotest.(check bool) "inverse of <" true
    (match gte with EBinOp (_, ">=", _) -> true | _ -> false);

  let gt = e_binop (v_var "x") ">" (v_num 5) in
  let lte = Expr.inverse gt in
  Alcotest.(check bool) "inverse of >" true
    (match lte with EBinOp (_, "<=", _) -> true | _ -> false)

let test_expr_flipped () =
  let e1 = e_binop (v_num 1) "<" (v_num 2) in
  let flipped = Expr.flipped e1 in
  Alcotest.(check bool) "< flips to >" true
    (match flipped with EBinOp (VNumber n, ">", VNumber m) ->
      Z.equal n (Z.of_int 2) && Z.equal m (Z.of_int 1) | _ -> false);

  let e2 = e_binop (v_num 1) "+" (v_num 2) in
  let flipped2 = Expr.flipped e2 in
  Alcotest.(check bool) "+ is commutative" true
    (match flipped2 with EBinOp (VNumber n, "+", VNumber m) ->
      Z.equal n (Z.of_int 2) && Z.equal m (Z.of_int 1) | _ -> false);

  (* Non-commutative operations should not flip *)
  let e3 = e_binop (v_num 1) "-" (v_num 2) in
  let flipped3 = Expr.flipped e3 in
  Alcotest.(check bool) "- doesn't flip" true
    (match flipped3 with EBinOp (VNumber n, "-", VNumber m) ->
      Z.equal n (Z.of_int 1) && Z.equal m (Z.of_int 2) | _ -> false)
...
let test_expr_subst () =
  ...
  Alcotest.(check bool) "substitution applied" true
    (match result with EBinOp (VNumber n, "+", VNumber m) ->
      Z.equal n (Z.of_int 5) && Z.equal m (Z.of_int 2) | _ -> false)
...
let test_expr_is_tautology () =
  ...
  Alcotest.(check bool) "x < x is not tautology" false (Expr.is_tautology not_taut)
...
let test_expr_is_contradiction () =
  ...
  Alcotest.(check bool) "x < x is contradiction" true (Expr.is_contradiction contr1);
  Alcotest.(check bool) "x > x is contradiction" true (Expr.is_contradiction contr2);
  Alcotest.(check bool) "x != x is contradiction" true (Expr.is_contradiction contr3);
  Alcotest.(check bool) "x = x is not contradiction" false (Expr.is_contradiction not_contr);

  (* Test with concrete numbers *)
  let false_eq = e_binop (v_num 1) "=" (v_num 2) in
  let true_eq = e_binop (v_num 1) "=" (v_num 1) in
  Alcotest.(check bool) "1 = 2 is contradiction" true (Expr.is_contradiction false_eq);
  Alcotest.(check bool) "1 = 1 is not contradiction" false (Expr.is_contradiction true_eq)
...
let suite =
  ( "Expressions",
    [
      ...
      Alcotest.test_case "Expr is_contradiction" `Quick test_expr_is_contradiction;
